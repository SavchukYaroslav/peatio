...
We are pleased to present Peatio Open Source latest release v1.9.0. This release includes significant new features, numerous functional fixes and enhancements:
...
We advice to use [minimalistic environment](#minimalistic-local-development-environment-with-docker-compose) if you want to develop only Peatio and don't touch processes interacting with other components.

Otherwise we advice to use [workbench based environment](#local-development-environment-with-workbench). Workbench is the recommended development, test and integration environment for new developers. It is the easiest way to setup development environment and run Peatio locally.
